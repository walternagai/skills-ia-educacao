...
- **Manual AvalIA** (`manual/`) — *AvalIA: Kit de Avaliação Formativa com Inteligência Artificial para Professores do Ensino Superior*: manual em LaTeX (10 capítulos e 2 apêndices, 80 páginas) com rota de 60 minutos para iniciantes, instalação do OpenCode, catálogo das skills e casos de uso validados (PDF compilado em `manual/main.pdf`)
...
> **Nota**: no Nível 5, "co-criação" descreve o processo pedagógico de explorar fronteiras com a IA — não confere autoria ou coautoria à ferramenta. A contribuição intelectual e a responsabilidade final permanecem humanas (ver o Guia de referência para uso de IAG da UNIFEI).
...
O manual *AvalIA: Kit de Avaliação Formativa com Inteligência Artificial para Professores do Ensino Superior* (`manual/`) é o guia completo do acervo (80 páginas): apresenta a Escala AIAS, instala o OpenCode passo a passo, cataloga as 62 skills e os 6 subagentes, traz casos de uso validados em sala e inclui apêndices para iniciantes (Git) e para manutenção do acervo. Para quem está começando, há rota de 60 minutos, kit inicial de 5 skills e textos prontos para o plano de ensino. Compile-o com `manual/compilar.sh` ou `make -C manual`; o PDF pronto está em `manual/main.pdf`.
